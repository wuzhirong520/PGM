...
def main(args, dataset, device):
    ...
    for epoch in range(0, args.num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            save_dir = os.path.join(str(args.result_path), str(args.dataset))
            os.makedirs(save_dir, exist_ok=True)
            torch.save(model.state_dict(), os.path.join(save_dir, str(args.dataset) + f'_model_{epoch + 1}.pt'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rp', '--result_path', type=str, default='./log/pretrain')
    args = parser.parse_args()

    device = torch.device("cuda:" + str(args.device)) if torch.cuda.is_available() else torch.device("cpu")

    import time
    wandb.init(project="PGM", name="pretrain-"+args.dataset+"-"+str((int)(time.time())),config=args)
